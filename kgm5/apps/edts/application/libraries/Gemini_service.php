...
class Gemini_service
{
    ...
    /**
     * Çok turlu sohbet: geçmiş mesajlarla birlikte yanıt üretir (token tasarrufu için son N mesaj kullanılabilir).
     *
     * @param array  $messages          Her eleman: array('role' => 'user'|'model', 'text' => string)
     * @param string $systemInstruction Sistem talimatı (opsiyonel)
     * @param float  $temperature       Yaratıcılık (opsiyonel)
     * @return string|false
     */
    public function chatWithHistory(array $messages, $systemInstruction = '', $temperature = null)
    {
        if (empty($messages)) {
            $this->lastError = 'Mesaj listesi boş olamaz.';
            return false;
        }
        ...
        if (empty($contents)) {
            $this->lastError = 'Geçerli mesaj bulunamadı.';
            return false;
        }
    ...
    public function summarize($data, $context = '', $language = 'tr')
    {
        ...
        $systemInstruction = "Sen bir kurumsal veri analiz asistanısın. "
            . "SADECE verilen verilerdeki bilgileri kullan. Veride olmayan hiçbir bilgi ekleme, tahmin etme veya uydurma. "
            . "Sayı, isim, tarih gibi tüm bilgiler veride geçenlerle birebir aynı olmalı. "
            . "Veride yer almayan bir konu için 'veride bulunmuyor' de; asla uygulama yapma. "
            . "Verilen verileri {$langText} olarak özetle. Kısa, net ve profesyonel dil kullan. "
            . "Yanıtını mutlaka TAMAMLA; cümle veya tablo ortasında kesme. "
            . "Yanıtını HTML formatında döndür. "
            . "Başlıklar için <h5>, <h6> etiketleri, listeler için <ul><li>, "
            . "önemli sayılar için <strong>, tablolar için <table class='table table-sm'> kullan. "
            . "Markdown veya düz metin KULLANMA, sadece HTML döndür.";
        ...
        $prompt .= "Veriler:\n{$data}\n\n";
        $prompt .= "Bu verilerin detaylı özetini HTML formatında oluştur. SADECE yukarıdaki veride geçen bilgileri kullan; veride olmayan hiçbir şey ekleme.";
