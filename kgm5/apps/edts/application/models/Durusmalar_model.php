...
class Durusmalar_model extends RT_Model
{
    ...
    /**
     * Takvim için tarih aralığına göre duruşma listesi (d_durusmatarihi Unix timestamp).
     * @param int $start Unix timestamp
     * @param int $end Unix timestamp
     * @param int|null $avukatId Sorumlu avukat id (null = tümü)
     * @return array
     */
    public function getEventsForCalendar($start, $end, $avukatId = null)
    {
        $start = (int) $start;
        $end   = (int) $end;
        $avukatWhere = '';
        if ($avukatId !== null && (int) $avukatId > 0) {
            $avukatWhere = ' AND d_avukatid = ' . (int) $avukatId;
        }
        $sql   = "SELECT d_id, d_dosyano, d_dosyaturu, d_mahkeme, d_durusmatarihi, d_esasno, d_memur, d_avukat, d_taraf, d_islem
                  FROM r8t_edts_durusmalar
                  WHERE d_status = 1 AND d_durusmatarihi IS NOT NULL AND d_durusmatarihi >= " . $start . " AND d_durusmatarihi <= " . $end . $avukatWhere . "
                  ORDER BY d_durusmatarihi ASC
                  LIMIT 1000";
        return $this->ek_query_all($sql) ?: array();
    }

    /**
     * Günlük veya haftalık yoğunluk (takvim analizi).
     * @param int $start Unix timestamp
     * @param int $end Unix timestamp
     * @param string $granularity 'day' veya 'week'
     * @param int|null $avukatId Sorumlu avukat id (null = tümü)
     * @return array [{ label, count }, ...]
     */
    public function getDensity($start, $end, $granularity = 'day', $avukatId = null)
    {
        $start = (int) $start;
        $end   = (int) $end;
        $avukatWhere = '';
        ...
        if ($granularity === 'week') {
            $sql = "SELECT YEARWEEK(FROM_UNIXTIME(d_durusmatarihi)) AS grp,
                    CONCAT('Hafta ', YEARWEEK(FROM_UNIXTIME(d_durusmatarihi))) AS label,
                    COUNT(*) AS cnt
                    FROM r8t_edts_durusmalar
                    WHERE d_status = 1 AND d_durusmatarihi IS NOT NULL AND d_durusmatarihi >= " . $start . " AND d_durusmatarihi <= " . $end . $avukatWhere . "
                    GROUP BY grp
            ...
            $sql = "SELECT DATE(FROM_UNIXTIME(d_durusmatarihi)) AS grp,
                    DATE_FORMAT(FROM_UNIXTIME(d_durusmatarihi), '%d.%m.%Y') AS label,
                    COUNT(*) AS cnt
                    FROM r8t_edts_durusmalar
                    WHERE d_status = 1 AND d_durusmatarihi IS NOT NULL AND d_durusmatarihi >= " . $start . " AND d_durusmatarihi <= " . $end . $avukatWhere . "
                    GROUP BY grp
        ...
        $out = array();
        foreach ($rows as $r) {
            $out[] = array('label' => $r->label, 'count' => (int) $r->cnt);
        }
        return $out;
    }
