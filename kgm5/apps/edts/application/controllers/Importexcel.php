...
class Importexcel extends CI_Controller
{
    ...
    public function doupload()
    {
        ...
        if(!empty($_FILES["dosya"]["name"])){
            ...
            if ( $xlsx = SimpleXLSX::parse('./upload/'.$dosya.'')) {
                $allRows = $xlsx->rows();
                if (!is_array($allRows)) {
                    $allRows = ($allRows instanceof \Traversable) ? iterator_to_array($allRows) : array();
                }
                $maxRows = 5000;
                if (count($allRows) > $maxRows) {
                    ...
                    $alert = array(
                        "title" => "Hata!",
                        "text"  => "Excel satır sayısı en fazla " . $maxRows . " olabilir. Lütfen dosyayı bölerek yükleyiniz.",
                        "type"  => "error"
                    );
                    $this->session->set_flashdata("alertToastr", $alert);
                    redirect(base_url("importexcel"));
                ...
                $addDate = dateToTimeFormat(date('Y-m-d H:i:s'), "Y-m-d H:i:s");
                $userId = $this->userData->userB->u_id;
                $batch = array();
                foreach ($allRows as $sutun) {
                    $batch[] = array(
                        "d_mahkeme" => isset($sutun[0]) ? $sutun[0] : '',
                        "d_esasno" => isset($sutun[1]) ? $sutun[1] : '',
                        "d_dosyaturu" => isset($sutun[2]) ? $sutun[2] : '',
                        "d_durusmatarihi" => isset($sutun[3]) ? dateToTimeFormat($sutun[3], "d.m.Y H:i:s") : 0,
                        "d_tarafbilgisi" => isset($sutun[4]) ? $sutun[4] : '',
                        "d_islem" => isset($sutun[5]) ? $sutun[5] : '',
                        "d_status" => 0,
                        "d_adduser" => $userId,
                        "d_adddate" => $addDate,
                    );
                ...
                foreach (array_chunk($batch, 100) as $chunk) {
                    if ($this->db->insert_batch("r8t_edts_importexcel", $chunk)) {
                        $say += count($chunk);
                    }
                }
    ...
    public function api_excelnewrecord() //EXCELLDEN MODAL SONRASI VERILERI EKLEME FONKSIYONU
    {
        ...
        $esasNoTrim = trim($_edtsEsasNo);
        $dosyaNoTrim = trim($_edtsDosyaNo);
        $dupSql = "SELECT d_id FROM r8t_edts_durusmalar WHERE d_status = 1 AND TRIM(IFNULL(d_esasno,'')) = ? AND TRIM(IFNULL(d_dosyano,'')) = ? LIMIT 1";
        $dupRow = $this->db->query($dupSql, array($esasNoTrim, $dosyaNoTrim))->row();
        if ($dupRow && !empty($dupRow->d_id)) {
            $_sonuc = new stdClass();
            $_sonuc->success = false;
            $_sonuc->code = 409;
            $_sonuc->duplicate = true;
            $_sonuc->existingId = (int) $dupRow->d_id;
            $_sonuc->description = 'Aynı esas no ve dosya no ile kayıt mevcut. Eski kaydı görüntüleyebilir veya silebilirsiniz.';
            $sonuc = json_encode($_sonuc);
            SetHeader(409);
            echo $sonuc;
            exit;
