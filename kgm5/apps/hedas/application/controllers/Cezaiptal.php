...
class Cezaiptal extends CI_Controller
{
    ...
    public function ara()
    {
        ...
        if (!isDbAllowedListModule()) {
            $alert = array(
                "title" => "Hata!",
                "text"  => "HEDAS | Ceza İptal Listesi Modülünü Görüntüleme Yetkiniz Bulunmuyor. Lütfen Sistem Yöneticinizden Yetki İsteyiniz.",
                "type"  => "error"
            );
            $this->session->set_flashdata("alertToastr", $alert);
            redirect(sys_url("home"));
        ...
        $cezaiptaltum = $this->cezaiptal_model->cezaIptalTum();

        $viewData = new stdClass();
        $viewData->viewFolder     = $this->viewFolder;
        $viewData->subViewFolder    = "ara";
        $viewData->userData             = $this->userData;
        $viewData->cezaiptaltum = $cezaiptaltum
        ;
        $this->load->view("{$this->viewFolder}/{$viewData->subViewFolder}/index", $viewData);
    }
    public function api_list()
    {
        ...
        if (in_array($_searchValue->ciEvrakDurum, array(-1, 0, 1, 2, 3, 4, 5, 6, 7)) === false) {
            $_sonuc =  new stdClass();
            $_sonuc->recordsTotal           = 0;
            $_sonuc->recordsFiltered        = 0;
            $_sonuc->data                   = array();
            $_sonuc->success                = false;
            $_sonuc->code                   = 200;
            $_sonuc->description            = "100 - Sorgu Parametreleri Eksik. Lütfen Sistem Yöneticinize Başvurunuz.";
            $sonuc = json_encode($_sonuc, JSON_UNESCAPED_UNICODE);
            SetHeader($_sonuc->code);
            echo $sonuc;
            exit;
        ...
        if (strlen(trim(replaceHtml_Slash($_searchValue->ciText))) > 0) {
            $sqlEk .= " AND (ci_plaka LIKE '%" . trim(replaceHtml_Slash($_searchValue->ciText)) . "%'";
            $sqlEk .= " OR ci_esasno LIKE '%" . trim(replaceHtml_Slash($_searchValue->ciText)) . "%'";
            $sqlEk .= " OR ci_kararno LIKE '%" . trim(replaceHtml_Slash($_searchValue->ciText)) . "%'";
            $sqlEk .= " OR ci_cezaserino LIKE '%" . trim(replaceHtml_Slash($_searchValue->ciText)) . "%'";
            $sqlEk .= " OR ci_kurumdosyano LIKE '%" . trim(replaceHtml_Slash($_searchValue->ciText)) . "%'";
            $sqlEk .= " OR ci_itirazeden LIKE '%" . trim(replaceHtml_Slash($_searchValue->ciText)) . "%'";
            $sqlEk .= " OR ci_icra LIKE '%" . trim(replaceHtml_Slash($_searchValue->ciText)) . "%'";
            $sqlEk .= " OR ci_mahkeme LIKE '%" . trim(replaceHtml_Slash($_searchValue->ciText)) . "%'";
            $sqlEk .= " OR ci_cezakonu LIKE '%" . trim(replaceHtml_Slash($_searchValue->ciText)) . "%'";
            $sqlEk .= " OR ci_davakonu LIKE '%" . trim(replaceHtml_Slash($_searchValue->ciText)) . "%'";
            $sqlEk .= " OR ci_aciklama LIKE '%" . trim(replaceHtml_Slash($_searchValue->ciText)) . "%'";
            $sqlEk .= " OR ci_tags LIKE '%" . trim(replaceHtml_Slash($_searchValue->ciText)) . "%')";
        }

        if (in_array($_searchValue->ciEvrakDurum, array(0, 1, 2, 3, 4, 5, 6, 7)) === true) {
            $sqlEk .= " AND ci_evrakdurum = '" . (int)trim(replaceHtml($_searchValue->ciEvrakDurum)) . "'";
        }
        ...
        $sql .= " ORDER BY ci_kurumdosyano desc";
    ...
    public function api_listDashboard()
    {
        ...
        $totalSql = "SELECT COUNT(ci_id) AS total
        FROM (
            SELECT ci_id
            FROM r8t_edys_cezaiptal
            WHERE ci_status = 1
            ORDER BY ci_id DESC
            LIMIT 5
        ) AS subquery";
    ...
    public function api_FormMahkemeSearch()
    {
        ...
        $sql = "SELECT mh_name FROM r8t_sys_mahkemeler WHERE mh_name LIKE '%Sulh%' AND mh_name LIKE '%" . $searchText . "%' GROUP BY mh_name";
        $items = $this->cezaiptal_model->ek_query_all($sql);
        $itemText = "";
        if ($items) {
            foreach ($items as $item) {
