...
var dosyaPreviewModalSaveasBtn;
...
var KTDosyaListServerSide = function () {
	...
	var handleDatatimePicker = function(element) {
		var startDef = moment().subtract(5, 'year').startOf('month');
		var endDef = moment();
		var initialRange = startDef.format('DD-MM-YYYY') + ' & ' + endDef.format('DD-MM-YYYY');
		$("#kt_table_dosya_datein").daterangepicker({
			opens: 'left',
			showDropdowns: true,
			startDate: startDef.format("DD-MM-YYYY"),
			endDate: endDef.format("DD-MM-YYYY"),
			ranges: {
				'Bugün': [moment(), moment()],
		...
		if (tarihPicker && (!tarihPicker.value || String(tarihPicker.value).indexOf(' & ') === -1)) {
			try { tarihPicker.value = initialRange; } catch (e) {}
		}
    ...
    var initDatatable = function () {
        var defaultAralik = moment().subtract(5, 'year').startOf('month').format('DD-MM-YYYY') + ' & ' + moment().format('DD-MM-YYYY');
        if (tarihPicker && tarihPicker.value && String(tarihPicker.value).indexOf(' & ') !== -1) {
            calisma_aralik = tarihPicker.value;
        } else {
            calisma_aralik = defaultAralik;
            if (tarihPicker) {
                try { tarihPicker.value = defaultAralik; } catch (e) {}
            }
        }
        ...
        dtDosyaList = $("#kt_content_dosya_list").DataTable({
            ...
            ajax: {
                ...
                data: function (d) {
                    d.search = d.search || {};
                    d.search.value = JSON.stringify(filterData);
                    return d;
                },
                error: function (xhr, textStatus, thrown) {
                    console.error('[DataTables kt_content_dosya_list] İstek hatası:', textStatus, thrown, xhr.status);
                }
            },
            columns: [
                // { data: null },
            ...
            columnDefs: [
				...
				},
                {
                    ...
                    render: function (data, type, row) {
                        var val = row._davaci || '';
                        return '<span class="d-inline-block text-truncate" style="max-width:150px" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-inverse" data-bs-placement="top" title="'+val+'">'+val+'</span>';
                    },
                },
                {
                    targets: 4,
                ...
                },
                {
                    ...
                    className: 'dt-nowrap',
                    render: function (data, type, row) {
                        var val = row._davakonuaciklama || '';
                        return '<span class="d-inline-block text-truncate" style="max-width:200px" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-inverse" data-bs-placement="top" title="'+val+'">'+val+'</span>';
                    },
                },
                {
                    targets: 6,
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        var val = row._esasno || '';
                        return '<span data-bs-toggle="tooltip" data-bs-custom-class="tooltip-inverse" data-bs-placement="top" title="'+val+'">'+val+'</span>';
                    },
                },
                {
                    targets: 8,
                ...
                },
                {
                    ...
                    className: 'text-left dt-nowrap',
                    render: function (data, type, row) {
                        var val = row._mevkiplaka || '';
                        return '<span class="d-inline-block text-truncate" style="max-width:120px" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-inverse" data-bs-placement="top" title="'+val+'">'+val+'</span>';
                    },
                },
                {
                    targets: 10,
        ...
        .on( 'select', function ( e, dt, type, indexes ) {
            ...
            $('#kt_modal_dosya_preview_saveas_btn').attr('data-id',rowData._id);
    ...
    var handleFilterDatatable = () => {
        ...
        filterButtonDosyaList.addEventListener('click', function (e) {
            ...
            filterData.dArsivNo              = araDLArsivNo.value;
    ...
    var handlePreviewSaveasButton = () => {
        dosyaPreviewModalSaveasBtn.addEventListener('click', function (e) {
            e.preventDefault();
            KTDosyaListServerSide.saveAsModal();
        });
    }
    ...
    var handleExcelExportButton = () => {
        ...
        dosyaExcelExportBtn.addEventListener('click', function (e) {
            ...
            var tarihFarki = 1;//zamanFarkiHesapla(ilkTarih, sonTarih);
    ...
    return {
        init: function () {
            ...
            dosyaPreviewModalSaveasBtn = document.getElementById('kt_modal_dosya_preview_saveas_btn');
            ...
            araDLArsivNo         = filterFormDosyaList.querySelector('[name="dlara_arsivno"]');
        ...
        saveAsModal: function(){
            var hostUrl = window.location.host;
            const baseUrlHost = "//"+hostUrl+"/apps/hedas/dosya/";
            console.log("save as girildi +"+baseUrlHost);
            var modulUrl = window.location.href;
            var dosyaId = $('#kt_modal_dosya_preview_saveas_btn').attr('data-id');
            var postData = {
                id: dosyaId
            }

            Swal.fire({
                text: "İlgili Kayıt  Farklı kaydedilecek. Emin misiniz?",
                icon: "warning",
                showCancelButton: true,
                buttonsStyling: false,
                confirmButtonText: "Farklı Kaydet",
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        dataType: "json",
                        url: baseUrlHost+"api_saveasdata",
