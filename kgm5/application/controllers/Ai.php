...
class Ai extends CI_Controller
{
    ...
    public function api_test()
    {
        ...
        $r->data->timestamp = date('Y-m-d H:i:s');
        $this->_out($r);
    }
    ...
    public function api_weekSummary()
    {
        try {
            ...
            $tarihFormatla = function ($tarih) use ($gunAdlari) {
                ...
                return date('d/m/Y', $ts) . ' - ' . $gunAdlari[(int) date('w', $ts)];
            };
            ...
            $tarihHaftaFormatla = function ($tarih) use ($aylar) {
                $ts = strtotime($tarih);
                if ($ts === false) return $tarih;
                return (int) date('j', $ts) . ' ' . $aylar[(int) date('n', $ts)] . ' ' . date('Y', $ts);
            };
            $haftaTarihMetni = '';
    ...
    /**
     * Kapasite planlaması için tahmine dayalı tek cümlelik uyarı döndürür (yoğun dönem, yoğun avukat, yoğun gün vb.).
     */
